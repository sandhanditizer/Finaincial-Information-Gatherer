...
def cleanData(data):
    """
    Takes in raw data and ensures that the data does not contain anything but negatives and decimals.
    ...
    if type(data) == dict:
        ...
        data = float(''.join(filter(lambda x: x.isdigit() or x in ['-', '.'], data)))
...
def reformatData(data):
    ...
    blocks = blocks[1:] # Removes column headers
    ...
    for i in range(len(blocks)):
        ...
        result.append({
                ...
                'Buy': cleanData(pieces[4]), 
                'Sell': cleanData(pieces[5]), 
                'Close': cleanData(pieces[6])
                    })
    return result
